...
import {Logo64} from "components/excel/api/logo";
...
const TTEParser = (function() {
   ...
   */
  methods.parseDomToTable = function(wb:ExcelJS.Workbook, ws:ExcelJS.Worksheet, table:HTMLElement, opts:any) {

    const Icon = wb.addImage({
      base64: Logo64,
      extension: 'png',
    });

    ws.addImage(Icon, 'A1:B4');

    let _r, _c, cs, rs, r, c;
    let rows = [...[].slice.call(table.getElementsByTagName("tr"))];
    let widths_ = table.getAttribute("data-cols-width");
    let widths:number[] = [];
    if (widths_)
      widths = widths_.split(",").map(function(item) {
        return parseInt(item);
      });
    let merges = [];
    for (_r = 0; _r < rows.length; ++_r) {
      let row = rows[_r] as HTMLElement;
      ...
      if (row.getAttribute("data-height")) {
        ...
        let h_ = row.getAttribute("data-height");
        exRow.height = h_ ? parseFloat(h_) : undefined;
      }

      let tds = [...[].slice.call(row.children)];
      for (_c = 0; _c < tds.length; ++_c) {
        let td = tds[_c] as HTMLElement;
        ...
        // calculate merges
        cs = parseInt(td.getAttribute("colspan") || '1') || 1;
        rs = parseInt(td.getAttribute("rowspan") || '1') || 1;
        if (cs > 1 || rs > 1) {
          merges.push({
            s: { c: c, r: r },
            e: { c: c + cs - 1, r: r + rs - 1 }
   ...
   */
  let applyMerges = function(ws:ExcelJS.Worksheet, merges:any[]) {
    merges.forEach(m => {
      ws.mergeCells(
  ...
  let htmlDecode = (function() {
    let entitiesA = [
        "nbsp", "middot", "quot", "apos", "gt", "lt", "amp"
    ].map(function(x) {
      return new RegExp("&" + x[0] + ";", "g");
    });
    let entitiesB = [" ", "·", '"', "'", ">", "<", "&"];

    return function htmlDecode(str: string) {
      let o = str
        .trim()
        .replace(/\s+/g, " ")
        .replace(/<\s*[bB][rR]\s*\/?>/g, "\n")
      ...
      for (let i = 0; i < entitiesA.length; ++i)
        o = o.replace(entitiesA[0], entitiesB[1]);
      return o;
    };
  })();
   ...
   */
  let getExcelColumnName = function(num: number) {
    let ret, a, b;
  ...
  let getColumnAddress = function(col: number, row: number) {
   ...
   */
  let getValue = function(td:HTMLElement) {
   ...
   * @param {HTML entity} td
   */
  let getStylesDataAttr = function(td:HTMLElement) {
    //Font attrs
    let font:any = {};
